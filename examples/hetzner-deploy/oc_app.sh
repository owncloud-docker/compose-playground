#! /bin/sh
# test_oc_app.sh - pull the latest version of an owncloud app from github and sugest a docker run command.
#
...
if [ -z "$1" ]; then
  ...
  echo "	$0 APPNAME"
  echo ""
  echo "retrieves the latest asset from github.com/$orga/APPNAME/releases"
  exit 1
fi
