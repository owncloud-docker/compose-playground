...
##
## Beware: OCIS_VERSION=v1.0.0-beta8 looks very much the same as v1.0.0-beta5 
## -- not sure, if the variable is effective here at all.
...
LOAD_SCRIPT <<EOF
  ...
  if [ -f ~/make_machine.bashrc ]; then
    ...
    ## We must either get the setattr right, or wait until ocis created the user at first login.
    # sleep 20; 
